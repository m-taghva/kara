...
SCRIPTS=("mrbench.py" "config_gen.py" "status_reporter.py" "monstaver.py" "analyzer.py" "report_recorder.py")
# Array of directories containing the scripts
SCRIPT_DIRS=("Mrbench" "Config_gen" "Status_reporter" "Monstaver" "Analyzer" "Report_recorder")

# Loop through each script and create a symbolic link in the destination directory
for ((i=0; i<${#SCRIPTS[@]}; i++)); do
    SCRIPT="${SCRIPTS[i]}"
    SCRIPT_DIR="${SCRIPT_DIRS[i]}"
    FULL_SCRIPT_PATH="$KARA_DIR/KARA/$SCRIPT_DIR/$SCRIPT"
    if [ ! -f "$FULL_SCRIPT_PATH" ]; then
        echo "Error: Script $SCRIPT not found in directory $SCRIPT_DIR"
        continue
    fi   
    ln_result=$(ln -s "$FULL_SCRIPT_PATH" "/usr/local/lib/$PYTHON_DIR/dist-packages/$SCRIPT" 2>&1)
    if [ $? -eq 0 ]; then
