...
for mc_server, config in data_loaded.get('influxdbs', {}).items(): 
    ...
    create_dir_command = f"ssh -p {ssh_port} {ssh_user}@{ip_influxdb} 'sudo mkdir -p {influx_restore_volume}/{directoryname}/dbs/restore/'"
    ...
    extract_command = f"ssh -p {ssh_port} {ssh_user}@{ip_influxdb} 'sudo docker exec -it -u root {container_name} tar -xf {influx_extracted_data}/{directoryname}/dbs/{container_name}.tar.gz -C {influx_restore_volume}/{directoryname}/dbs/restore/'"
    ...
    check_command = f"ssh -p {ssh_port} {ssh_user}@{ip_influxdb} 'sudo docker exec -it {container_name} influx -execute 'show databases''"
    try:
    ...
    if output is not None and database in output:
       restore_command = f"ssh -p {ssh_port} {ssh_user}@{ip_influxdb} 'sudo docker exec -it {container_name} influxd restore -portable -db {database} -newdb {tempdb_name} {influx_restore_volume}/{directoryname}/dbs/restore/'"
       ...
       if restore_exit_code == 1:
          ...
          # Merge phase
          merge_command = f"ssh -p {ssh_port} {ssh_user}@{ip_influxdb} 'sudo docker exec -it {container_name} influx -execute 'SELECT * INTO \"{database}\".autogen.:MEASUREMENT FROM \"{tempdb_name}\".autogen./.*/ GROUP BY *''"
          ...
          # Drop tmp db
          drop_tmp_command = f"ssh -p {ssh_port} {ssh_user}@{ip_influxdb} 'sudo docker exec -it {container_name} influx -execute 'drop database {tempdb_name}''"
         ...
         restore_command = f"ssh -p {ssh_port} {ssh_user}@{ip_influxdb} 'sudo docker exec -it {container_name} influxd restore -portable -db {database} {influx_restore_volume}/{directoryname}/dbs/restore/' "
         restore_process = subprocess.run(restore_command, shell=True)
         restore_exit_code = restore_process.returncode
         if restore_exit_code == 1:
            print("\033[91mRestore failed.\033[0m")
