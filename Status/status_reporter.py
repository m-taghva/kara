...
##### Constants #####
START_TIME_SUM = 10 # increase your test start time 
END_TIME_SUBTRACT = 10 # decrease your test end time
...
output_csv_str = ["Host_alias"] # name of first column in csv
csvi = 0
# Loop through each combination of time range, host, IP, PORT, and execute the curl command
for mc_server, values in data_loaded.get('influxdbs', {}).items():
    IP = values['ip_influx']
    PORT = values['port_influx']
    DATABASE = values['database_influx']
    HOST = values['host_monster']
    ALIAS = values['alias_monster'] if values['alias_monster'] and len(values['alias_monster']) > 1 else HOST
    ...
    output_csv_str.append(ALIAS)  # value inside first column of csv
    csvi += 1
    for metric_file, metric_operation in metric_operation_mapping.items():
        if os.path.isfile(metric_file):
            ...
            for metric_name in metrics:
                ...
                query1_curl_command = f'curl -sG "http://{IP}:{PORT}/query" --data-urlencode "db={DATABASE}" --data-urlencode "q=SELECT {metric_operation}(\\"value\\") FROM \\"{metric_name}\\" WHERE (\\"host\\" =~ /^{HOST}$/) AND time >= \'{start_time_utc}\' AND time <= \'{end_time_utc}\' fill(none)"'
                ...
                query2_curl_command = f'curl -sG "http://{IP}:{PORT}/query" --data-urlencode "db={DATABASE}" --data-urlencode "q=SELECT {metric_operation}(\\"value\\") FROM /{metric_name}/ WHERE (\\"host\\" =~ /^{HOST}$/) AND time >= \'{start_time_utc}\' AND time <= \'{end_time_utc}\' GROUP BY time({TIME_GROUP}s) fill(none)"'
